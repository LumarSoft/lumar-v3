...
export async function GET(request: Request) {
  ...
  if (items.length === 0) {
    return NextResponse.json({ ok: true, sent: false, message: "Nada por vencer" })
  }
  ...
  const rows = items
    .map((it) => {
      const when = it.days < 0 ? `vencido hace ${Math.abs(it.days)}d` : it.days === 0 ? "vence hoy" : `en ${it.days} días`
      const color = it.days <= 0 ? "#f87171" : it.days <= 3 ? "#fb923c" : "#eab308"
      return `<tr>
