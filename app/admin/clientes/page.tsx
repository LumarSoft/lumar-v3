    ...
    () => [
      {
        ...
        render: (row: DocRecord) => {
          const sum = cobros.data
            .filter((c) => c.cliente === row.cliente && c.categoria === "Recurrente mensual")
            .reduce((acc, c) => acc + toNum(c.monto), 0)
          return sum > 0 ? (
            <span className="whitespace-nowrap font-medium">{formatARS(sum)}/mes</span>
          ) : (
      ...
      },
      {
        ...
        render: (row: DocRecord) => {
          const list = cobros.data.filter(
            (c) => c.cliente === row.cliente && c.categoria === "Fijo desarrollo",
          )
