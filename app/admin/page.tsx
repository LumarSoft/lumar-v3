...
export default function DashboardPage() {
      ...
      const when =
        days < 0
          ...
          : days === 0
            ? "vence hoy"
            : `vence en ${days} día(s)`;
      const key = isRecurrente(c)
        ? `cobro_${c.id}_${cobroPeriodo(c)}`
        : `cobro_${c.id}`;
  ...
  const stats = useMemo(() => {
    ...
    const progress = Math.min(
      100,
      Math.round((recurrenciaUsd / META_USD) * 100),
    );
    ...
    const cobradoMes = cobros.data
      .filter(
        (c) => inMonth(c.vencimiento, year, month) && cobroPaidForMonth(c),
      )
    ...
    const vencimientosMes = vencimientos.data.filter((v) =>
      inMonth(v.vencimiento, year, month),
    );
    const gastoMes = vencimientosMes.reduce((a, v) => a + toNum(v.monto), 0);

    const tareasAbiertas = tareas.data.filter(
      (t) => t.estado !== "Hecho",
    ).length;
    ...
    const revisiones = clientes.data
      ...
      .map((c) => ({
        id: String(c.id),
        cliente: c.cliente,
        proximaRevision: c.proximaRevision,
        _days: daysUntil(String(c.proximaRevision)) ?? 9999,
      }))
      .filter((c) => c._days <= 30)
      .sort((a, b) => a._days - b._days)
      .slice(0, 6);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Datos de {selected.label.toLowerCase()}. Norte: 570 → 3.000 USD/mes
            recurrente.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Kpi
          ...
          value={formatARS(stats.cobradoMes)}
          sub={selected.label}
        />
        <Kpi
          icon={Receipt}
        ...
        <Kpi
          ...
          value={String(stats.tareasAbiertas)}
          sub="sin terminar"
        />
      </div>
      ...
      <Card className="border-border/60">
        <CardHeader className="pb-3">
          <CardTitle className="text-base">
            Progreso a la meta · 3.000 USD/mes recurrente
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="flex items-end justify-between">
            <span className="text-2xl font-semibold">
              {formatUSD(stats.recurrenciaUsd)}
            </span>
          ...
          <p className="text-xs text-muted-foreground">
            Solo cuenta el cobro recurrente (no los pagos puntuales de
            desarrollo). Equivale a 1.000 USD por socio.
          </p>
        </CardContent>
      </Card>
          ...
          <CardContent className="space-y-2">
            {stats.revisiones.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                Nada que revisar en los próximos 30 días.
              </p>
            ) : (
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-xs text-muted-foreground">
                        {[
                          v.tipo,
                          v.vencimiento
                            ? formatDate(String(v.vencimiento))
                            : null,
                        ]
                          .filter(Boolean)
                          .join(" · ")}
                ...
                <Badge
                  ...
                  className={cn(
                    "border",
                      ...
                      ? OPTION_COLOR_CLASSES.green
                      : OPTION_COLOR_CLASSES.gray,
                  )}
                >
