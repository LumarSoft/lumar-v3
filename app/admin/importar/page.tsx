...
import { SEED_CLIENTES, SEED_PROYECTOS, SEED_COBROS } from "@/lib/admin/seed-data"
...
export default function ImportarPage() {
  ...
  async function importAll() {
    ...
    try {
      const seed = async (
        ...
        const existing = new Set(coll.data.map((d) => String(d[titleKey]).toLowerCase()))
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Importar datos iniciales</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Carga los clientes, proyectos y cobros recurrentes que ya teníamos relevados. Es seguro
          correrlo más de una vez: no duplica lo que ya existe.
        </p>
      </div>
      ...
      <Card className="border-border/60">
        ...
        <CardContent className="space-y-4">
          <ul className="space-y-1 text-sm text-muted-foreground">
            <li>• Clientes: Mutual, Uesevi, Heroica, John BOT, John Web (montos y estado).</li>
            <li>• Proyectos: los 6 frentes, vinculados por nombre de cliente.</li>
            <li>• Cobros: el recurrente mensual de cada cliente.</li>
          </ul>
          <Button onClick={importAll} disabled={running || loading} size="lg">
            {done ? <Check className="size-4" /> : <DownloadCloud className="size-4" />}
            {running ? "Importando…" : done ? "Importado" : "Importar todo"}
          </Button>
          {done ? <p className="text-sm text-emerald-400">{done}</p> : null}
          <p className="text-xs text-muted-foreground">
            Después de importar, podés pulir cada registro en su sección y borrar este ítem del menú
            (línea “Importar” en <code>components/admin/nav-items.tsx</code>).
          </p>
        </CardContent>
      </Card>
    </div>
