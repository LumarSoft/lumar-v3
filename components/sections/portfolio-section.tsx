...
export function PortfolioSection() {
  ...
  useEffect(() => {
    if (isInViewRef.current) {
      window.dispatchEvent(new CustomEvent('portfolioAccent', { detail: projects[activeIdx].accent }))
    }
  }, [activeIdx])

  const goTo = (i: number) => setActiveIdx(Math.max(0, Math.min(projects.length - 1, i)))
  ...
  return (
    <section ref={sectionRef} id="portfolio" className="relative bg-zinc-950 overflow-hidden py-20 md:py-28">
      ...
      <div className="relative z-10 px-6 md:px-12 lg:px-16 mb-10 md:mb-14 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-3">
            <motion.div className="h-px w-10" animate={{ backgroundColor: active.accent }} transition={{ duration: 0.7 }} />
            <span className="text-[11px] font-semibold uppercase tracking-[0.25em] text-zinc-500">Portafolio</span>
          </div>
          <h2 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-zinc-100 leading-[0.95] tracking-tight">
            Casos reales,{' '}
            <span className="text-zinc-500">no demos.</span>
          </h2>
        </div>
        ...
        <div className="flex items-center gap-4">
          <span className="hidden md:block text-xs text-zinc-600">
            <motion.span animate={{ color: active.accent }} transition={{ duration: 0.6 }} className="font-mono font-bold tabular-nums">
              {active.number}
            </motion.span>
      ...
      >
        <motion.div
          ...
          style={{ x, gap: GAP }}
          drag="x"
          dragConstraints={{ left: targetX(projects.length - 1), right: targetX(0) }}
          dragElastic={0.12}
          onDragEnd={onDragEnd}
        >
          {projects.map((p, i) => {
            ...
            return (
              <motion.article
                ...
                style={{ width: cardW || '86vw', borderColor: `rgba(${p.accentRgb},0.18)` }}
                animate={{
              ...
              >
                {/* Accent top bar */}
                <div className="absolute top-0 inset-x-0 h-0.5 z-20" style={{ backgroundColor: p.accent }} />
                ...
                <div className="relative h-[42%] min-h-[180px] overflow-hidden">
                  <Image
                    ...
                    draggable={false}
                  />
                  <div className="absolute inset-0" style={{ backgroundColor: `rgba(${p.accentRgb},0.08)` }} />
                  ...
                  <div className="absolute top-4 left-4 flex items-center gap-2 rounded-full bg-zinc-950/70 px-3 py-1 backdrop-blur-sm">
                    <span className="font-mono text-xs font-bold tabular-nums" style={{ color: p.accent }}>{p.number}</span>
                ...
                <div className="flex flex-1 flex-col gap-3 p-6 md:p-7">
                  <div>
                    <h3 className="font-display text-2xl md:text-3xl font-bold text-zinc-100 leading-tight">{p.client}</h3>
                    <p className="text-zinc-600 text-xs mt-0.5">{p.industry}</p>
                  </div>

                  <p className="text-zinc-500 text-sm italic leading-relaxed">"{p.problem}"</p>

                  <div className="flex items-center gap-2.5">
                    <div className="h-px w-4 flex-none" style={{ backgroundColor: p.accent }} />
                  ...
                  <ul className="mt-auto flex flex-col gap-1.5 pt-1">
                    {p.impact.map((item) => (
                      <li key={item} className="flex items-start gap-2 text-[13px] text-zinc-300">
                        <span className="shrink-0 mt-0.5 text-xs font-bold" style={{ color: p.accent }}>↗</span>
                        {item}
                      </li>
                    ))}
                  </ul>
                  ...
                  {p.stack.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-3 border-t border-zinc-800/60">
                      {p.stack.map((tech) => (
                        <span key={tech} className="text-[11px] text-zinc-400 bg-zinc-800/40 border border-zinc-700/40 px-2.5 py-0.5 rounded-md">
                          {tech}
                        </span>
                      ))}
                    </div>
      ...
      <div className="relative z-10 px-6 md:px-12 lg:px-16 mt-10 flex items-center justify-between gap-6">
        <div className="flex items-center gap-2 text-zinc-600">
          ...
          <span className="text-[11px] uppercase tracking-widest">Arrastrá para explorar</span>
        </div>

        <div className="flex items-center gap-2">
          {projects.map((p, i) => (
            <button key={p.number} onClick={() => goTo(i)} aria-label={`Ir a ${p.client}`} className="py-2">
              <motion.div
                animate={{
